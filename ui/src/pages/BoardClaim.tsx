...
export function BoardClaimPage() {
  ...
  if (!sessionQuery.data) {
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-lg font-semibold">{t("cliAuth.signInRequired")}</h1>
          <p className="mt-2 text-sm text-muted-foreground">
      ...
      <div className="rounded-lg border border-border bg-card p-6">
        ...
        {claimMutation.error && (
          <p className="mt-3 text-sm text-destructive">
            {claimMutation.error instanceof Error ? claimMutation.error.message : t("boardClaim.error")}
          </p>
        )}
        ...
        >
          {claimMutation.isPending ? t("boardClaim.claiming") : t("boardClaim.action")}
        </Button>
      </div>
    </div>
  );
